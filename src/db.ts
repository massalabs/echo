...
export class EchoDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(3).stores({
      ...
      messages: '++id, contactId, type, direction, status, timestamp, encrypted',
      conversations: '++id, contactId, lastMessageTimestamp, unreadCount, isPinned, isArchived',
  ...
  async getMessagesForContact(contactId: number, limit = 50): Promise<Message[]> {
  ...
  async addMessage(message: Omit<Message, 'id'>): Promise<number> {
    ...
    if (conversation) {
      await this.conversations.update(conversation.id!, {
        ...
        unreadCount: message.direction === 'incoming' ? conversation.unreadCount + 1 : conversation.unreadCount,
        updatedAt: new Date()
      })
    } else {
