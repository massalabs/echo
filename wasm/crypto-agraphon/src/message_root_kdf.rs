...
mod tests {
    ...
    fn test_message_root_kdf_deterministic() {
        ...
        let kdf1 = MessageRootKdf::new(&randomness, &self_mk, &peer_mk, &ss, &ct, &Role::Initiator);
        let kdf2 = MessageRootKdf::new(&randomness, &self_mk, &peer_mk, &ss, &ct, &Role::Initiator);
    ...
    fn test_message_root_kdf_different_roles() {
        ...
        let kdf_initiator =
            MessageRootKdf::new(&randomness, &self_mk, &peer_mk, &ss, &ct, &Role::Initiator);
        let kdf_responder =
            MessageRootKdf::new(&randomness, &self_mk, &peer_mk, &ss, &ct, &Role::Responder);
    ...
    fn test_message_root_kdf_different_randomness() {
        ...
        let kdf1 =
            MessageRootKdf::new(&randomness1, &self_mk, &peer_mk, &ss, &ct, &Role::Initiator);
        let kdf2 =
            MessageRootKdf::new(&randomness2, &self_mk, &peer_mk, &ss, &ct, &Role::Initiator);

        // Different randomness should produce different keys (multi-device safety)
        assert_ne!(kdf1.cipher_key.as_bytes(), kdf2.cipher_key.as_bytes());
        assert_ne!(kdf1.cipher_nonce.as_bytes(), kdf2.cipher_nonce.as_bytes());
    }
}
